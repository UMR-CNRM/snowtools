...
def parse_command_line():
    ...
    parser.add_argument("-a", "--assimdates", nargs="+", help="List of assimilation dates", type=str, required=False)
...
def fetch_prep_footprints(block: str, assimdate: str, nmembers: int):
    prep = vortex.input(
        kind="PREP",
        vapp=args.vapp,
        vconf=args.vconf,
...
def clear_preps(block: str, assimdate: str, nmembers: int):
    prep_footprints = fetch_prep_footprints(block=block, assimdate=assimdate, nmembers=nmembers)
    if not args.keep_existing_prep:
        for fic in prep_footprints:
            fic.delete()
...
def wait_mandatory_input(block, assimdate, nmembers, walltime):
    ...
    start = Date.now()
    timer = Date.now()
    prep = fetch_prep_footprints(block=block, assimdate=assimdate, nmembers=nmembers)
    # if not args.keep_existing_prep:
    #     for fic in prep:
    #         fic.delete()
    time.sleep(1)
...
def mkjob_list_commands(taskname, njobs=17, mb0=0, datebegin=None, dateend=None, date=None):
    ...
    if njobs == 1:
        ...
        for job_number in range(mb0, mb0 + njobs):
            mkjob_list.append(
                mkjob_command(
                    jobname=f"{jobname}_mb{job_number}",
                    taskname=taskname,
                    datebegin=datebegin,
                    dateend=dateend,
                    date=date,
