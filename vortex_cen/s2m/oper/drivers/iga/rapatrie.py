# -*- coding:Utf-8 -*-
"""
Monthly SAFRAN renalysis
"""

__all__ = []

from iga.tools.op import InputReportContext, OutputReportContext
...
class Reanalyses(Task, CENTaskMixIn):
    """
    Task : Reanalyses
    =================

    Daily generation (12H run) of an archive of SAFRAN guess files containing all guess from last 01/08
    to be used by the monthly SAFRAN reanalysis.
    1 execution per S2M domain.

    Inputs
    ------
    - Guess archive from J-1 [or J-2, J-3, ..., J-31]
    - SAFRAN analysis guess files from J-1 [plus J-2, J-3, ..., J-31 if necessary]
    - [RESCUE MODE] SAFRAN forecast guess files from J-1 [plus J-2, J-3, ..., J-31 if necessary]

    Outputs
    -------
    - Guess archive covering 01/08 --> today
    """

    MANDATORY_CONFIGURATION_VARIABLES = [
        "xpid",
        "rundate+help=Date of run;choices=YYYYMMDD12;type=str or Date",  # used in the "get_period" method
    ...
    def refill(self):
        ...
        if 'refill' in self.steps:  # Difference with IGA
