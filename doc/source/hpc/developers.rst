...
Finaly, it is possible to document the action of a given configuration variable on other configuration variables.
For example, in the "surfex" driver of the Crocus/deterministic configuration, if the optional "climground" configuration variable is set to "True" a "PREP.nc" file is looked for so the "prep" description becomes an option.
