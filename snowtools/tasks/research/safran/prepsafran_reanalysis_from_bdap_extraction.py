...
class PrepSafran(Task, S2MTaskMixIn):
    ...
    def process(self):
        ...
        if 'early-fetch' in self.steps or 'fetch' in self.steps:
            ...
            while datebegin < self.conf.dateend:

                dateend = datebegin.replace(year=datebegin.year + 1)

                self.sh.title('Toolbox input ARPEGE')
                tb02 = toolbox.input(
                    role           = 'Gridpoint',
                    kind           = 'packedguess',
                    local          = '[enddate:ymd6h]/ARPEGE_[begindate:ymd6h]_[enddate:ymd6h].tar',
                    experiment     = 'reanalysis2020.3@vernaym',
                    cutoff         = 'assimilation',
                    block          = 'guess',
                    nativefmt      = 'tar',
