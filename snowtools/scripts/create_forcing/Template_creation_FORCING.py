...
parser.add_argument("-o", "--output", help="Output netcdf file", default="FORCING_from_my_obs.nc", dest="outfile")
...
parser.add_argument("--meta", help="add global attributes in form attname=attribute_text. Can be evoced multiple times", dest="metadata", action="append")
...
station_data = {
    "LAT": float(args.LAT),
    "LON": float(args.LON),
    "UREF": float(args.UREF),
    "ZREF": float(args.ZREF),
    "ZS": float(args.ZS),
    "aspect": float(args.aspect),
    "slope": float(args.slope),
    "FRC_TIME_STP": float(args.FRC_TIME_STP),
}
...
default_meta_data = [
    ...
    "id=FORCING ID",
    "contributor_name=People 1, People 2, People 3",
    "contributor_role=People 1 collected the observation; People 2 made some modelisation; People 3 made some coffee  (huge contribution)",
]
###################
# treat meta data
##################
...
meteo_data["Qair"] = Thermo(["v", "c"], dict(P=meteo_data["PSurf"], Huw=meteo_data["HUMREL"], T=meteo_data["Tair"], rc=0)).get("qv")
...
if bool(args.RADPART) == True:
    ...
    meteo_data["DIR_SWdown"], meteo_data["SCA_SWdown"] = sun().directdiffus(
        meteo_data["DIR_SWdown"], meteo_data.index, station_data["LAT"], station_data["LON"], station_data["slope"], station_data["aspect"], site=args.RADPART_PARAMS
    )
...
meteo_data = meteo_data[["CO2air", "DIR_SWdown", "HUMREL", "LWdown", "NEB", "PSurf", "Qair", "Rainf", "SCA_SWdown", "Snowf", "Tair", "Wind", "Wind_DIR"]]
...
with netCDF4.Dataset(args.outfile, "w", format="NETCDF4_CLASSIC") as fic_forcing:
    ...
    List_nom_time_nbpoint = ["CO2air", "DIR_SWdown", "HUMREL", "LWdown", "NEB", "PSurf", "Qair", "Rainf", "SCA_SWdown", "Snowf", "Tair", "Wind", "Wind_DIR"]

    List_unite_time_nbpoint = ["kg/m3", "W/m2", "%", "W/m2", "between 0 and 1", "Pa", "Kg/Kg", "kg/m2/s", "W/m2", "kg/m2/s", "K", "m/s", "deg"]

    List_longname_time_nbpoint = [
    ...
    List_unite_nbpoint = ["degrees_north", "degrees_east", "m", "m", "m", "degrees from north", "degrees from horizontal"]

    List_longname_nbpoint = ["latitude", "longitude", "Reference_Height_for_Wind", "Reference_Height", "altitude", "slope aspect", "slope angle"]

    # Creating 2 dimensions variables
    for i in range(len(List_nom_time_nbpoint)):
        fic_nc = fic_forcing.createVariable(List_nom_time_nbpoint[i], "f", ("time", "Number_of_points"), fill_value=fill_value)
