...
def replace_obs_tar(tar_name):
    ...
    for file_type in ls_file_type:
        for obs_file in glob.glob(f'{file_type}????????'):
            ...
            column_widths = [6, 6, 6, 10, 10, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 1, 8, 1, 1, 1, 3, 1, 3, 1, 3, 1, 2, 1,
                    2, 1, 6, 1, 1, 1, 1, 3]
            df = pd.read_fwf(obs_file, header=None, widths=column_widths, converters={18: str})
            df = df.rename(columns={0: "type_poste_actuel", 1: "lat", 2: "lon", 3: "Station_Name", 4: "num_poste",
                6: "Temperature", 9: "ZS", 18: "time"})
            ...
            if file_type == "S":
                ...
                # reconstituted_obs_i = reconstructed_data.sel(time=date)
                ...
                timestep = [pd.to_datetime(a_date).strftime("%y%m%d%H") for a_date in
                        reconstituted_obs_i["time"].values]
            ...
            if len(reconstituted_obs_i.time) > 0:
                ...
                if file_type == "S":
                    # obs_added["time"] = date.yymdh
                    obs_added = obs_added.reset_index().set_index(["num_poste", "time"])
                obs_added = obs_added[["t", "type_poste_actuel", "Station_Name", "lat", "lon", "ZS"]].rename(columns=
                        {"t": "Temperature"})
